...
// Sidebar order follows the business flow: Lead â Contact â Class/Activity â Payment â Management

const NAV = [

  { id: 'dashboard',   label: '××©×××¨×',      ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg> },

  { id: 'sales',       label: '××××¨××ª',       ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> },

  { id: 'contacts',    label: '××§××××ª',       ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },

  { id: 'classes',     label: '××××× ××§××¨×¡××', ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },

  { id: 'financial',   label: '×¤×× × ×¡×× âª',    ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg> },

  { id: 'tasks',       label: '××©××××ª',      ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> },

  { id: 'instructors', label: '×××¨××××',       ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/><path d="M12 12v9"/><path d="M9 15l3-3 3 3"/></svg> },

  { id: 'students',    label: '×ª××××××',       ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg> },

  { id: 'activities',  label: '×¤×× × ××××ª',      ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="3" width="6" height="6" rx="1"/><rect x="9" y="3" width="6" height="6" rx="1"/><rect x="16" y="3" width="6" height="6" rx="1"/><rect x="2" y="12" width="6" height="9" rx="1"/><rect x="9" y="12" width="13" height="4" rx="1"/><rect x="9" y="18" width="6" height="3" rx="1"/></svg> },

  { id: 'reports',     label: '×××××ª ð',      ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/><path d="M2 20h20"/></svg> },

  { id: 'invoices',    label: '××©××× ×××ª',      ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> },

  { id: 'automations', label: '××××××¦×××ª',     ico: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg> },
...
export function MobileBottomNav({ page, setPage, tasks, leads, onMore }) {
  ...
  const items = [
    ...
    { id: 'dashboard', label: '××©×××¨×',  badge: 0,         ico: NAV[0].ico },

    { id: 'sales',     label: '××××¨××ª',  badge: leadBadge,  ico: NAV[1].ico },

    { id: 'contacts',  label: '××§××××ª',  badge: 0,         ico: NAV[2].ico },

    { id: 'classes',   label: '×××××',   badge: 0,         ico: NAV[3].ico },

    { id: 'more',      label: '×¢××',     badge: 0,
  ...
  return (
    ...
    <>
      ...
      <nav className="mob-nav" role="navigation" aria-label="× ×××× ×¨××©×">
        ...
        {items.map(item => (
          ...
          >
            ...
            <span className="mob-nav-ico">
              ...
              {item.badge > 0 && <span className="mob-badge" aria-label={`${item.badge} ××ª×¨×××ª`}>{item.badge}</span>}
  ...
  return (
    ...
    <div className={`sidebar ${mobOpen ? 'mob-open' : ''}`}>
      ...
      <div className="sidebar-logo">
        ...
        <div className="logo-t"><h1>WIN CRM</h1><p>××××ª× ×××</p></div>
      ...
      <nav className="sidebar-nav">
        ...
        {NAV.map(n => (
          ...
          <button key={n.id} className={`nav-item ${page === n.id ? 'active' : ''}`} onClick={() => handleNav(n.id)}>
            ...
            {n.id === 'sales'       && (atRiskLeads + newLeads) > 0 && <span className="nav-badge" style={{ background: atRiskLeads > 0 ? '#ef4444' : '#f97316' }}>{atRiskLeads > 0 ? `â ${atRiskLeads}` : newLeads}</span>}
