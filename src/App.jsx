...
// ââ Instructor portal (public, no auth needed) âââââââââââââââââââââââââââââ
...
export default function App() {
  ...
  // After agent creates items â reload the relevant data so pages update instantly
  ...
  // Activity logger â optimistic + async persist
  ...
  const addDeal = async data => {
    ...
    if (data.contactId && row) logAct(data.contactId, `× ××¦×¨× ×¢×¡×§×: ${data.title}`)
  ...
  const updateDeal = async (id, data, prevStage) => {
    ...
    if (data.contactId && prevStage && prevStage !== data.stage) {
      ...
      logAct(data.contactId, `×¢×¡×§× ×¢×××× ×: ${data.title} â ${st?.label}`)
  ...
  const toggleTask = async id => {
    ...
    if (updated?.completed && updated?.contactId) logAct(updated.contactId, `××©××× ×××©×××: ${updated.title}`)
  ...
  // Only show full-screen loading on initial auth check â NOT on data refreshes
